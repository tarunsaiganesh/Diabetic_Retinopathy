...
train_df = raw_train_df.groupby(['level', 'eye']).apply(lambda x: x.sample(2720, replace = True)
                                                      ).reset_index(drop = True)
print('New Data Size:', train_df.shape[0], 'Old Size:', raw_train_df.shape[0])

#train_df.to_csv("trainDFnew.csv")
...
def tf_augmentor(out_size,
    ...
    load_ops = tf_image_loader(out_size = intermediate_size, 
                               ...
                               on_batch=False)
    def batch_ops(X, y):
        print("into batch ops in augumentor")
    ...
    def _create_pipeline(in_ds):
        print("into create pipeline")
        batch_ds = in_ds.map(load_ops, num_parallel_calls=4).batch(batch_size)
       # print(batch_ds.size())
        return batch_ds.map(batch_ops)
    return _create_pipeline               # returns a batch(48) of resized images from in_ds
...
valid_idg = tf_augmentor(out_size = IMG_SIZE, color_mode = 'rgb', 
                         ...
                         rotation_range = 0,
                        batch_size = batch_size)
print("end augumentation")
################################
print("start preprocess train")
...
retina_model.fit_generator(train_gen, 
                           ...
                           validation_steps = valid_df.shape[0]//batch_size,
                             ...
                             workers = 4, # tf-generators are not thread-safe
